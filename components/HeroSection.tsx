...
import { ArrowRight, Sparkles, Mic, Type, Volume2, FlipHorizontal, Smartphone } from 'lucide-react';
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700">免费 · 无需登录 · 浏览器直接用</span>
          </motion.div>
          ...
          >
            适合 iPhone / iPad 录视频使用
            <br />
            支持镜像模式、滚动调速、字号行距自定义，草稿自动保存
          </motion.p>
          ...
          >
            {[
              { icon: Type, label: '字号/行距/边距', desc: '自由调节' },
              { icon: Volume2, label: '滚动速度', desc: '10-200 px/s' },
              { icon: FlipHorizontal, label: '镜像模式', desc: '适配提词器硬件' },
              { icon: Smartphone, label: '手机可用', desc: 'iPhone/iPad 友好' },
            ].map((feature) => (
              <motion.div
                key={feature.label}
                whileHover={{ y: -5 }}
